...
export default function ServicesPage() {
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      const hasCategory = selectedCategory && selectedCategory !== 0;

      // Case 1: No search term → Use all providers endpoint (even with category)
      if (!hasSearchTerm) {
      ...
      // Case 2: Has search term → Use recommendation API
  ...
  const searchWithFilters = async () => {
    try {
      ...
      console.log(`✅ Found ${results.length} providers via recommendation`);
