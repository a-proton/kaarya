...
function LoginContent() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      ...
      // Store tokens (also sets accessToken cookie via storeTokens)
      storeTokens(accessToken, refreshToken || undefined);

      // Set userType cookie for middleware redirect logic
      document.cookie = `userType=${response.data?.user?.user_type || ""}; path=/; SameSite=Lax`;
      ...
      <div className="w-full max-w-md">
        <div className="rounded-2xl shadow-xl p-8 bg-white border border-neutral-100">
          ...
          <div className="flex rounded-xl p-1.5 mb-6 bg-neutral-100 shadow-inner">
            ...
            >
              <i className="fas fa-briefcase mr-2"></i>Provider
            </button>
          </div>
